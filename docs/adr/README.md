...
| [023](ADR-023-deal-remainder-and-turn-order.md) | Every card is dealt with the remainder going to seats in ascending order, and the current leader's seat stored on `game_session` | Accepted | No — both decisions are recorded ahead of the code. The remainder deal and the derived opening lead land in EOP-14 Slice A/C; `current_leader_seat` arrives in Liquibase changeset `004` in Slice B, which does not exist yet |
