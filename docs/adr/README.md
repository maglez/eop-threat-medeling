...
| [022](ADR-022-agent-model-tier-governance.md) | Model tiers allocated by Definition-of-Done role, not by artefact type | Accepted | Yes — `@tester-api` pinned to `{env:MODEL_B}`, all five DoD gates on `MODEL_A`/`MODEL_B`, and each carries a Sign-off Contract in its own definition. The narrowing of the Separation Invariant to exclude test code is recorded rather than denied; splitting `AUDITOR_AGENTS` in `tools/agent-trace.py` remains outstanding follow-up |
