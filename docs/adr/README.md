...
| [023](ADR-023-deal-remainder-and-turn-order.md) | Every card is dealt with the remainder going to seats in ascending order, the current leader's seat stored on `game_session`, and `TrickPlay.components` stored one row per component in `trick_play_component` | Accepted | Partly — the domain half of decision 1 shipped in EOP-14 Slice A: `Hands.deal` round-robins the whole deck so the remainder falls on the lowest seats, `Hands.openingLeaderSeat` derives the opening lead with no rank written down, and `Trick.nextLeaderSeat(seatsHoldingCards)` passes the lead on when the winner has run out of cards. Decision 2 is **built in storage only** as of EOP-14 Slice B: `current_leader_seat` lands in Liquibase changeset `004`, and nothing is wired into a use case or an endpoint yet — Slice C owns that. The components decision, added by the 2026-08-12 amendment, is likewise **built in storage**: the domain layer of its 20-element bound shipped in Slice A, `trick_play_component` with `PRIMARY KEY (trick_play_id, ordinal)` and `CHECK (ordinal <= 19)` ships in changeset `004` with migration tests pinning the CHECK to `TrickPlay.MAX_COMPONENTS`, and the `@Size(max = 20)` boundary layer waits for Slice D. Slice B also creates `hand`, `hand_card`, `trick`, `trick_play` and `trick_play_component` with 6 unique constraints and 10 foreign keys; `hand_card` keyed on `(hand_id, card_id)` is what makes the `20, 20, 19, 19` deal storable at all and was missing from this ADR's own changeset inventory until the 2026-08-12 amendment. **Seat binding *is* in storage**, reversing an earlier deferral on the same day: `uq_player_id_seat` on `player (id, seat_order)` plus `fk_hand_player_seat` and `fk_trick_play_player_seat` make a hand or a play at a seat its player does not hold unrepresentable, and the single-column keys to `player(id)` are subsumed rather than omitted. **Cross-session enforcement remains deliberately not in storage** and is Slice C's to carry — on the play path, the deal path (`hand` has the identical gap) and the delete path, and it blocks cards as well as seats. Audit then found a **fourth** gap that is not cross-session at all: `fk_trick_winner_play` confines a trick's winner to no trick and no session. The ADR now names **four** Slice C obligations, each naming its exception type and the HTTP status the refusal carries — 403/404 for the seat and membership checks, 409 for the duplicate constraints, 422 for the wrong-trick winner, and a fourth obligation to extend the adapters' constraint-name translation so those violations stop answering 500 (@tester-api, 2026-08-12) — and note that nothing in Slice B is reachable by application code — the five new tables have no entity, so the containment is the absence of a mapping and not a feature flag — see the reversal and the remaining deferrals in the ADR |
