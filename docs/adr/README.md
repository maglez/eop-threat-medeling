...
| [023](ADR-023-deal-remainder-and-turn-order.md) | Every card is dealt with the remainder going to seats in ascending order, and the current leader's seat stored on `game_session` | Accepted | Partly — the domain half of decision 1 shipped in EOP-14 Slice A: `Hands.deal` round-robins the whole deck so the remainder falls on the lowest seats, `Hands.openingLeaderSeat` derives the opening lead with no rank written down, and `Trick.nextLeaderSeat(seatsHoldingCards)` passes the lead on when the winner has run out of cards. Decision 2 is not built: `current_leader_seat` arrives in Liquibase changeset `004` in Slice B, and nothing is wired into a use case or an endpoint yet |
