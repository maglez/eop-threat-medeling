...
- **Keep the default `auto` scope and create no `graphify.yaml`,** declining the recommendation Graphify itself writes into `.graphify/scope.json`: *"Use --scope all or graphify.yaml inputs.corpus for a knowledge-base folder."* That string is emitted whenever `resolved_mode` is `committed`; it is not advice tailored to this repository, and both halves fail on the evidence.
  - `--scope all` would ingest the **27,989** ignored files alongside the 68 committed ones. **99.5% of them are `node_modules`** — 24,194 under `tools/graphify` (Graphify's own dependency tree) and 3,652 under `.opencode/node_modules` — and the remainder is `target/` build output plus `.graphify/` itself, so the graph would ingest its own output. Against retrieval that already returns 91% of the graph for one keyword query, that is strictly worse.
  - `inputs.corpus` aims at a real gap but cannot close it. **52 of 94 tracked files are Markdown, and the graph contains 8 distinct `source_file` values, none of them Markdown** — this Blueprint, all eleven ADRs, `SETUP.md` and every agent definition contribute zero nodes. The cause is not scope: those files are already tracked and already in scope. Code takes a deterministic local tree-sitter pass, whereas documentation is extracted by a *semantic pass through a platform model*, and we deliberately run with no model backend. Changing the candidate set would still yield zero doc nodes. Creating the file would also switch Graphify into **ontology profile mode** — type constraints, citation rules, review statuses, candidate queues, reconciliation, a studio SPA — a large machine to operate for 88 lines of Java.
  - **Revisit trigger:** getting documentation into the graph is a cost decision — a model backend plus `--description-mode direct` — not a configuration change. Reopen it when the token spend is justified, not before. (`--scope tracked` is currently identical to `committed`, since `excluded_untracked_count` is 0.)
