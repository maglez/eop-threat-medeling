...
4. Slice C **must extend the constraint-name translation in the persistence adapters**, and this
   ...
   **This table is not an inventory of how each exception is raised, only of how each *constraint* is translated.** Three exceptions also arise without any constraint being violated, and no row here can cover them: `HandAlreadyDealtException` from the deal-once gate, when `current_leader_seat` is already set; its deliberate mirror `HandNotDealtException`, when the column is still null and a trick or a play needs a turn order that no deal has established; and `TrickAlreadyResolvedException` from the `winner_play_id IS NULL` predicate on a replayed resolution. Naming one half of that mirror pair and omitting the other is how an architecture gate caught this paragraph undercounting at two. Both are recorded in [ADR-020](ADR-020-session-concurrency-control.md); this sentence exists so a reader who reaches for this table as the complete list is sent there instead of concluding the list is complete.
