...
4. Slice C **must extend the constraint-name translation in the persistence adapters**, and this
   ...
   **This table is not an inventory of how each exception is raised, only of how each *constraint* is translated,** and it must not be read as the former. Six exceptions on the trick-play write paths arise with no constraint violated at all, so no row here can cover them. Three come from a rows-affected count of zero on a conditional statement, which is not a violation of anything: `HandAlreadyDealtException` from the deal-once gate, when `current_leader_seat` is already set; its deliberate mirror `HandNotDealtException`, when the column is still null and a trick or a play needs a turn order no deal has established; and `TrickAlreadyResolvedException` from the `winner_play_id IS NULL` predicate on a replayed resolution. Those three are recorded in [ADR-020](ADR-020-session-concurrency-control.md), which owns the conditional statements they come from. Naming one half of the `HandAlreadyDealt`/`HandNotDealt` mirror pair and omitting the other is how an architecture gate caught this paragraph undercounting at two, and then at three. The other three come from checks the adapter makes itself, before or instead of reaching a constraint: `PlayerNotInSessionException` and `NotYourSeatException` from the session-scoped seat read in `assertSeated`, which runs *before* the insert `fk_hand_player_seat` would otherwise refuse — so `NotYourSeatException` is simultaneously a row of this table and a non-constraint origin, and the superseding note below is why the constraint route became a 500 backstop rather than the 403 the row promises — and `CardNotInHandException` from a conditional DELETE matching no row.
