...
4. Slice C **must extend the constraint-name translation in the persistence adapters**, and this
   ...
   **This table is not an inventory of how each exception is raised, only of how each *constraint* is translated,** and it must not be read as the former. **Nine** exceptions on the trick-play write paths arise with no constraint violated at all, so no row here can cover any of them. `TrickPlayExceptionOriginTest` pins that number against the adapter source, because this paragraph miscounted four times running — at two, then three, then six — and a count that four careful readers got wrong needs a machine rather than a fifth.
