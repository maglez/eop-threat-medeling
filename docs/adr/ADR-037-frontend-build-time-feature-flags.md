...
**Status:** Accepted (EOP-11, 2026-08-16; amended 2026-08-18 by EOP-77 and EOP-78, 2026-08-22 by EOP-145, and 2026-08-24 by EOP-168 — see the amendment paragraphs under Consequences)
...
**`VITE_GAME_SCREEN_ENABLED` supplied to Docker via `ARG`/`ENV` (EOP-78 amendment, 2026-08-18).** `ui/Dockerfile` now declares `ARG VITE_GAME_SCREEN_ENABLED=false` and `ENV VITE_GAME_SCREEN_ENABLED=$VITE_GAME_SCREEN_ENABLED` immediately before `RUN npm run build`. This is the only viable channel: `ui/.dockerignore` is default-deny and does not allow-list `.env*`, so no `.env` file reaches the build context. The `ARG` default is `false` (fail-closed, consistent with `feature-flags.md`). The CI pipeline (`ci.yml`) and `scripts/update-local.sh` both pass `--build-arg VITE_GAME_SCREEN_ENABLED=true` explicitly to enable the game screen in the built artefact; a plain `docker build` without that argument produces a dark build. The `ENV` line is necessary in addition to `ARG`: Vite's `loadEnv` reads from `process.env`, not from Docker build metadata, so `ARG` alone would not propagate the value into `import.meta.env`. The `ENV` is scoped to the `build` stage only and does not leak into the `caddy:2-alpine` runtime stage.
...
**`VITE_CARD_MAGNIFIER_ENABLED` is the second live instance, and EOP-145's ticket specified the wrong mechanism (EOP-145 amendment, 2026-08-22).** The card magnifier — a 2× CSS lens over hand cards and trick-zone cards — is gated by `VITE_CARD_MAGNIFIER_ENABLED`, declared in `ui/src/vite-env.d.ts`, supplied to Docker by an `ARG`/`ENV` pair defaulting to `false`, and passed `--build-arg VITE_CARD_MAGNIFIER_ENABLED=true` by both `.github/workflows/ci.yml` and `scripts/update-local.sh`, exactly as the EOP-78 paragraph above describes for the game screen. The story as filed asked instead for `eop.features.card-magnifier` under `application.yml`, with an acceptance criterion that "the bean is absent from the application context" when the flag is off. Neither is implementable, for two independent reasons, and both are the reasons this ADR exists: a purely presentational CSS lens has **no Spring bean** for `@ConditionalOnProperty` to remove, which is precisely the browser-side gap ADR-013 is deliberately not extended across; and a Spring property is invisible to a bundle Caddy serves as static assets from a separate path (ADR-017), so no amount of server configuration could reach it. This is recorded rather than quietly corrected because the ticket was written in good faith from the backend convention in `feature-flags.md`, and the next story to gate front-end behaviour will make the same mistake unless the two mechanisms are visibly distinguished. Two further points specific to this instance. It is the first flag read **below** the view router — in `GameScreen.tsx` rather than `App.tsx` — which does not weaken the "gate at entry, not at the button" clause above: the magnifier has no route to gate, so its entry point genuinely *is* the card component that owns the pointer handlers, and the flag is still read once at component scope and threaded down as a prop rather than consulted at each interaction. Reading it at *module* scope would have been the real error, because a module-scope read is captured at import time and defeats `vi.stubEnv` — the flag would then be untestable in both positions, which `feature-flags.md` requires. Accordingly `CardMagnifier.test.tsx` tests three positions, not two: `'true'`, `'false'`, and unset (the empty string), the last confirming that the `=== 'true'` comparison is what makes an absent variable fail closed. Finally, shipping the flag `=true` from CI in the same story that introduces it is a deliberate, disclosed deviation from the merge-dark default in `feature-flags.md`, justified by the change being wholly presentational with no server surface and by all five Definition-of-Done gates running inside that story; the `ARG` default remains `false`, so a plain `docker build` still produces a dark build. One measured property of that dark build is worth stating in the same breath, because "the flag is off" invites the stronger reading: **a flag-off build still ships the lens code.** Building `ui/` three times during EOP-145's review — with `VITE_CARD_MAGNIFIER_ENABLED=true`, with `=false`, and with the variable unset — emitted `dist/assets/index-*.js` at **198,662 bytes in all three cases**, with the `eop-card-lens` class name present in every bundle and the flag surviving only as the minified prop value `magnifierEnabled:!0` versus `magnifierEnabled:!1`. The `=false` and unset builds produced an identical content hash. Vite substitutes the value and then minifies, but it does not prune the component the value guards, so the flag controls whether the lens **renders**, never whether it is **compiled**. This is the same shape as the `import.meta.glob` paragraph above and has the same consequence: the flag is an affordance control, not a security boundary, and nothing confidential may ever be placed behind one.

**`VITE_FOLLOW_SUIT_HINT_ENABLED` is the third live instance, and issue #299 repeated EOP-145's wrong-mechanism error (EOP-168 amendment, 2026-08-24).** The proactive follow-suit hint — an inline `govuk-hint` line telling the active player which suit they must follow, so they learn the constraint before a play is rejected with a 422 — is gated by `VITE_FOLLOW_SUIT_HINT_ENABLED`, declared in `ui/src/vite-env.d.ts` as `string`, read once at component scope in `GameScreen.tsx` and threaded into `FollowSuitHint` as its `enabled` prop. The wiring is the same eight-site set the EOP-78 amendment established: the declaration, the component-scope read, the `ARG`/`ENV` pair in `ui/Dockerfile` defaulting to `false`, `--build-arg …=true` in both `.github/workflows/ci.yml` and `scripts/update-local.sh`, a documented entry in `ui/.env.example`, and `vi.stubEnv` coverage. The amendment above predicted that "the next story to gate front-end behaviour will make the same mistake unless the two mechanisms are visibly distinguished", and it did: issue #299 asked for `eop.features.follow-suit-hint` in `application.yml`, a `@ConditionalOnProperty` gate with `havingValue = "true"`, and an entry in `src/test/resources/feature-flag-registry.yml`. None of that was implementable — a presentational hint has no Spring bean to omit, a Spring property is invisible to a bundle Caddy serves as static assets (ADR-017), and `FeatureFlagRegistryTest` derives its flag set from the shipped YAML *and* from the compiled `@ConditionalOnProperty` sites and requires both to equal the registry, so a `VITE_` row would have failed `./mvnw verify` from both directions at once. The delivered story therefore touched no `application.yml`, no `@ConditionalOnProperty` and no registry file, and the ticket's Definition of Done is wrong on this point rather than unmet. Recording a *third* instance of the same confusion was the argument for distinguishing the two mechanisms in `.opencode/rules/feature-flags.md` itself, which until this story mentioned the `VITE_*` flags only as "a separate mechanism" in a closing parenthesis. That distinction is now written there as a dedicated section, and this story is where it landed rather than a follow-up: a decision procedure keyed on whether the flag decides that a Spring bean exists; the four further wiring sites a front-end flag needs beyond its declaration; an explicit statement that neither mechanism is available as a fallback for the other, with `FeatureFlagRegistryTest` failing in both directions; and the disclosure-boundary warning. The point of putting it there is reach — `.opencode/rules/` is loaded into every agent by one glob, so a future ticket now meets the distinction in context rather than only on following a link to this ADR.

Two refinements this instance adds to the pattern. First, the gate lives **inside** `FollowSuitHint`, which returns `null` when `enabled` is false, rather than in a `{flag && …}` conditional at the call site. This is the "evaluate the flag where the feature is entered" rule taken one step further: it makes the component the single choke point, so no future caller can render the hint while bypassing the flag. Second, the hint is suppressed when the player's hand holds no card of the led suit, because the domain rule is follow-suit-*if-able* (`Trick.java:201`, `Trick.java:215`) — a hint reading "you must play a tampering card" shown to a player holding none would state a falsehood. That fourth condition is a deliberate strengthening beyond issue #299's three acceptance scenarios, and the suit name is rendered with the repository's existing `.toLowerCase().replace(/_/g, ' ')` convention (four of five in-repo call sites) rather than the title case the ticket's example used, so the hint reads "a denial of service card". The three flag positions (`'true'`, `'false'`, unset) are exercised with `vi.stubEnv` in `GameScreen.test.tsx`, the only place `import.meta.env` is read; `FollowSuitHint.test.tsx` covers the prop logic in isolation, and its own header records why it cannot cover the flag. One measurement caveat: the production bundle is 199,810 bytes with the flag on, but the flag-off and unset positions were not re-measured for this story, so the byte-identical property recorded above continues to rest on EOP-145's evidence rather than on new evidence from this one.
