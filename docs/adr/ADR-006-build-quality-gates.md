...
> **Amendment, 2026-08-20 (EOP-120): a sixth gate and a sixth pinned plugin — Javadoc completeness, enforced by two tools rather than one.**
>
...
> - **The gate is incremental locally, and its own bookkeeping once failed the build.** At `61806db` a review agent observed `./mvnw verify` exit 1 with `Unable to compute stale date: target/reports/apidocs/options` — on a clean tree whose Java and Javadoc were both correct. Cause: the plugin deletes its `options`/`packages`/`argfile` working files after generating, while `StaleHelper`'s staleness record still stats `options`; in the state *apidocs present, `target/maven-javadoc-plugin-stale-data.txt` absent*, computing the record fails. **`debug=true` is the fix** — it makes those files persist — and it is configuration whose purpose is invisible from its name, so do not remove it as debug scaffolding. Verified by reproducing the exact failing state (`rm target/maven-javadoc-plugin-stale-data.txt` with `apidocs/` present) and getting `BUILD SUCCESS` where it previously exited 1.
> - **A warm local run may skip the analysis, and the log does not admit it clearly.** `Skipping javadoc generation, everything is up to date.` means the goal did no work. Do not read the goal's presence in a warm log as proof it re-analysed your edit: the staleness record lists jars and top-level output-directory files and contains **no `.java` path at all** (measured: 116 recorded paths, 93 `.jar`, zero `.java`), because `StaleHelper.walk` uses a non-recursive `Files.newDirectoryStream`. A comment-only edit therefore need not invalidate it. CI is unaffected and is the guarantee that matters — `.github/workflows/ci.yml` runs on a fresh runner with no `target/`, so **every merge is gated on a cold, full analysis**. Locally, run `./mvnw clean verify` before pushing a documentation-only change.
>
