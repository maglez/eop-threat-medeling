...
> **Amended 2026-09-06 (`EOP-000`).** One name in that list was never enforced and has been removed: `transition_issue` is now **allowed** for the Product Owner, which may move a ticket through the workflow. The remaining denies — `assign_issue`, `add_worklog`, the sprint and version tools, `delete_issue`, `remove_*` and `move_*` — are unchanged, so the boundary narrows by exactly one capability rather than dissolving.
>
> The reason is that the deny was decorative. `atlassian_jira_update_issue` is granted by the `atlassian_jira_*` wildcard, and the MCP server routes a status change through it: the `update_issue` tool parses the caller's `fields` JSON and splats it into `jira.update_issue(**kwargs)`, that method intercepts a `status` key and delegates to `_update_issue_with_status`, which resolves the transition by name or id and calls `set_issue_status_by_transition_id`. The tool's own docstring says "including changing status". So a single `update_issue` call with `fields='{"status": "Done"}'` transitioned a ticket while `transition_issue` sat denied beside it, and the agent needed no ingenuity to find that path — it is the ordinary way to edit a field.
>
> Closing the hole instead was considered and rejected. It would have meant denying `update_issue`, which is the tool this ADR's own decision exists to grant: without it the Product Owner can file a ticket and never refine one, which is most of the requirements role. There is no narrower deny available, because the permission layer matches tool names and cannot see a key inside a JSON argument. Granting the capability honestly was therefore the only option that left the configuration truthful.
>
> The general lesson is ADR-065's, met a third time and from a new direction. There, a permission block and the prose beside it disagreed about the world; here, two entries *within one permission block* disagreed with each other, one of them silently overriding the other. A deny that a sibling allow can walk around is worse than an absent rule, because a reader — human or agent — takes it as a boundary and reasons from it. When a rule cannot be enforced at the layer it is written in, move the boundary to where it holds rather than leaving the unenforceable rule in place as documentation of an intention.
