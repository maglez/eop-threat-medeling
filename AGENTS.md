...
- **A review gate must not share weights with an agent that writes production code.** `tester-unit-and-quality` was moved from `MODEL_C` to `MODEL_B` for this reason: `MODEL_A` writes Java, `MODEL_C` writes migrations and CI/IaC, `MODEL_E` writes the front-end, and `MODEL_B` authors no production code at all. On `MODEL_C` that agent needed three dispatches to return a verdict and once recommended merging a red build. The other half of that fix is in the agent definitions themselves: all five gate agents now carry a **Sign-off Contract** (a mandatory final `VERDICT: APPROVE`/`VERDICT: REJECT` line, severity-tagged findings with `file:line`, never end with a question, never approve a red build) and a **read-only-while-reviewing** rule. Both were previously enforced only by whatever the dispatching prompt happened to say, which is why they were skipped.
