...
- **Front-end: shipped, not pending.** `ui/` is a tracked React + TypeScript + Vite front end with GOV.UK Design System CSS (ADR-009), and it is built, tested, containerised and served. It is the *only* front end — put UI work inside it rather than scaffolding a second one or picking a different toolchain. Every claim here is checkable against a tracked file. **Stack —** `ui/package.json`: `react ^18.3.1` (**18, not the 19 ADR-009's Stack components section claims**), `typescript ^5.6.3`, `vite ^5.4.9`, `govuk-frontend ^5.7.1`, `engines.node >= 22`; the GOV.UK assets are copied in by `ui/scripts/copy-govuk-assets.mjs`, wired as the `prebuild`/`predev` scripts. **Location —** components in `ui/src/components/`, entry `ui/index.html` → `ui/src/main.tsx`, strict-mode TypeScript (`ui/tsconfig.json`). The typed DTO layer and every `fetch` wrapper live in the single module `ui/src/api.ts`: there is **no** `ui/src/types/`, `pages/`, `hooks/` or `services/` directory, so do not go looking for the layout ADR-009 sketches (corrected in its 2026-08-19 amendment). **Build —** `ui/vite.config.ts` emits to `ui/dist/` and runs a dev server on **`:5371`** (not Vite's default `:5173`) proxying `/api` and `/health` to Spring Boot on `:8080`; `npm run verify` in `ui/` chains typecheck, ESLint, Vitest and a production build, and `.github/workflows/ci.yml` runs those as a dedicated `ui` job separate from the Java `build` job. **Serving —** `ui/Dockerfile` builds the bundle with Node and serves it with Caddy, `ui/Caddyfile` serves the built assets at `/` and reverse-proxies `/api/*` and `/health` to the API so both sit on one origin and no CORS configuration exists anywhere in the project (ADR-017), and `compose.app.yml` composes that image alongside the API
