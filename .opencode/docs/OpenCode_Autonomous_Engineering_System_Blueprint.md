...
> **`autoUpdate: true` in `.opencode/dcp.jsonc` is inert here, and that is the intended outcome.** The key is real — it is declared in DCP's own `lib/config.ts` and defaults to `true` — but its documented scope is narrower than its name suggests: DCP's `README.md` says it updates an npm-installed DCP when a newer `latest` appears, and that **version-locked plugin specs are not updated**. Our spec carries an exact version (`@tarquinen/opencode-dcp@3.1.15`), so the switch never fires. Do not read the key as a second, competing source of truth that silently drifts past the pin; it *confirms* `SETUP.md`'s rule that the exact pins in the `plugin` array are authoritative and nowhere else. It would start mattering only if the spec were ever loosened to a range or a bare name — which is reason enough not to loosen it.
