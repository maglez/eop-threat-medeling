...
> **A stale `2.0.10` copy is vendored at `.opencode/node_modules/opencode-supermemory` and is NOT what loads.** OpenCode resolves npm plugins through `Npm.add()` into `~/.cache/opencode/packages/<spec>/`, ignoring `.opencode/node_modules` (190 MB, gitignored, left over from a manual install). `.opencode/package.json` and `.opencode/package-lock.json` are equally misleading — they still declare `opencode-supermemory: ^2.0.10` and lock `0.6.5` for the goal plugin, because neither was updated when those versions were bumped. Both are now **untracked** (`.opencode/.gitignore` had always listed them, but `.gitignore` does not apply to files already committed, so it took an explicit `git rm --cached`); they remain on disk and are inert for plugin loading. Do not trust any of the three as evidence of the running version — the `plugin` array in `.opencode/opencode.json` is the single source of truth. See also the retired `npm audit` re-check in `SETUP.md` §"Security Notes — Accepted Risks", which those manifests used to feed.
