...
| `MODEL_F` | `opencode/minimax-m2.5` | MiniMax | — | Architecture Guardian, Security Auditor — unprobed on the Zen route, see §3.4.1 |
...
| `MODEL_F` | `amazon-bedrock/minimax.minimax-m2.5` | MiniMax | **Audit** — the two gates that review `MODEL_A`'s own output |
...
| `MODEL_F` (gate) | @architecture-guardian, @security-auditor — the two gates that review `MODEL_A`'s own output. This is the `MODEL_E` precedent inverted: there, one model behind two variables so each can be repointed independently; here, two agents behind one variable because their requirements are identical and every extra variable multiplies drift across the tier tables |
