...
>
> - **Family-independent** — reviewer and author sit in different model families or reasoning architectures. This is what the paragraph above requires and the only degree that defends against a *shared training bias*: a blind spot common to every model in a family survives any amount of review from within it. On the current Bedrock mapping there are **two** family boundaries: `MODEL_C`/`MODEL_E` (`qwen.qwen3-coder-480b-a35b-v1:0`) against `MODEL_A`/`MODEL_B`/`MODEL_D` (all Anthropic Claude), and — added 2026-08-21 by [ADR-046](../../docs/adr/ADR-046-gate-model-capability-floor.md) — `MODEL_F` (`minimax.minimax-m2.5`), which sits outside both.
...
| @code-reviewer (Audit) | Static Code Review & SOLID Compliance | `{env:MODEL_F}` | MiniMax | Audit | 0.1 |
...
Agent frontmatter and the top-level config reference abstract environment variable names (`{env:MODEL_A}` through `{env:MODEL_F}`) rather than direct model IDs. The actual model ID is resolved at runtime from the corresponding variable in `.env`:
...
- `MODEL_F` — the audit tier for every gate that must be family-independent of every tier which authored what it reviews: `@architecture-guardian`, `@security-auditor`, and `@code-reviewer` since 2026-08-24 ([ADR-058](../../docs/adr/ADR-058-code-reviewer-moves-to-model-f.md)), which is three of the five Definition-of-Done gates. Do not describe it as "the two gates that review `MODEL_A`'s own output" — that was its narrower founding purpose, and `@code-reviewer` also reviews test code authored on `MODEL_B`, so the family requirement holds against both classes. It must be family-independent of `MODEL_A`/`MODEL_B`/`MODEL_D` *and* of `MODEL_C`/`MODEL_E`, and may only be pointed at a model that has passed the two-stage probe in [ADR-046](../../docs/adr/ADR-046-gate-model-capability-floor.md) with its verdict recorded in §3.4.1.

This indirection lets the operator switch between OpenCode Zen and Amazon Bedrock (or any future provider) by changing the six `MODEL_*` values in `.env` — no agent file or config changes needed.

#### Allocated Models

Six abstract model names map to the real provider-specific model IDs. All are non-deprecated as of 2026-07-27; prices are USD per 1M tokens (input / output) for the OpenCode Zen variant.
...
| `MODEL_F` | `opencode/minimax-m2.5` | MiniMax | — | Architecture Guardian, Security Auditor, Code Reviewer — unprobed on the Zen route, see §3.4.1 |
...
| `MODEL_F` | `amazon-bedrock/minimax.minimax-m2.5` | MiniMax | **Audit** — three of the five gates: Architecture Guardian, Security Auditor, Code Reviewer |
...
###### Gate-candidate probe results — code-review capability, `eu-west-2`, 2026-08-24 (EOP-178)

**A second record required by [ADR-046](../../docs/adr/ADR-046-gate-model-capability-floor.md) clause 4.** The 2026-08-21 block above cleared `minimax.minimax-m2.5` for the gate work it then held — architecture review and security audit. Moving `@code-reviewer` onto that tier is a *new* gate pin, so clause 4 requires its own passing verdict rather than inheriting that one, and code review is a different capability: an architecture reviewer reasons over prose and structure, whereas a code reviewer must cite `file:line` against source it has actually read.

The target was `src/main/java/org/maglez/eop/adapter/security/SecureRandomDeckShuffler.java`, 58 lines, chosen because its javadoc pre-empts the three findings a pattern-matching reviewer invents for a shuffler — modulo bias, a hand-rolled swap loop, and mutation of the caller's list — so a model that guesses reports a defect that is not there, while a model that reads sees each one already addressed. Ground truth, established independently by reading the file and by `wc -l`: `shuffle` is declared on line **52**, the single line performing the shuffle is line **55**, `Collections.shuffle(shuffled, random);`, and the file contains no `%` operator and no hand-written swap loop. Both stages were run against `eu.anthropic.claude-sonnet-4-6` as a control, as the 2026-08-21 block did, and the file was chosen partly because it is untouched by the concurrent EOP-175 branch and therefore has identical line numbers in every worktree.

| Stage | What it asks | Ground truth | `minimax.minimax-m2.5` | Control `eu.anthropic.claude-sonnet-4-6` |
|---|---|---|---|---|
| 1 — reads what it claims to read | Read the file, reply with the bare line number on which `shuffle` is declared | `52` | **PASS** — `→ Read`, then `52` | PASS — `→ Read`, then `52` |
| 2 — gate work under the Sign-off Contract | Review `shuffle()` for shuffle-bias defects; emit `QUOTE:`, `LINE:`, `DEFECT:`, then a terminal `VERDICT:` line | line `55`, `Collections.shuffle(shuffled, random);`, defect `ABSENT` | **PASS** — correct quote, correct line, `DEFECT: ABSENT`, terminal `VERDICT: APPROVE` | PASS — same, but wrapped the quote in backticks despite being told verbatim and nothing else |

**Verdict: `minimax.minimax-m2.5` is cleared for `@code-reviewer`.** Two observations beyond the bare pass. It did not take the fabrication bait: `ABSENT` is the correct answer and `PRESENT` is the tempting one, and reporting a defect that is not there is exactly what `nvidia.nemotron-super-3-120b` and `mistral.magistral-small-2509` were rejected for above. And on Sign-off Contract *shape* it was marginally cleaner than the incumbent it replaces — the control added markdown backticks to a quote it had been told to render verbatim; MiniMax did not. Read this as clearing the floor, not as evidence of superiority: it is a two-stage probe rather than a track record, so ADR-046's accepted cost 2 still stands, now over a third gate rather than two.
...
| `MODEL_F` (gate) | @architecture-guardian, @security-auditor, @code-reviewer — three of the five Definition-of-Done gates, being those that must be family-independent of every authoring tier. This is the `MODEL_E` precedent inverted: there, one model behind two variables so each can be repointed independently; here, three agents behind one variable because their requirements are identical and every extra variable multiplies drift across the tier tables — which is why [ADR-058](../../docs/adr/ADR-058-code-reviewer-moves-to-model-f.md) added an occupant in 2026-08-24 rather than a seventh variable |
...
The reason is §7.8's `external_directory` fallback, not tidiness. `/tmp` and `~/Documents/Tech/AI/…` are both outside the worktree, so each touch raises a prompt that **blocks** until answered, and a human who is not watching the pane costs the run minutes of wall-clock time. A path inside the worktree is not an external directory, so the check is never consulted. This is why the convention needs **no permission entry at all**: adding `"external_directory": {"*": "ask", "~/Documents/Tech/AI/**": "allow"}` was drafted and then rejected, because it widens a boundary that is working correctly in order to solve a problem that a different directory solves for free.

`.gitignore` carries `.tmp/` with the constraints inline, and `AGENTS.md` states the convention so a fresh session reaches for it without being told.
