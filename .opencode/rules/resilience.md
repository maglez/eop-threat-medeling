...
- This is a weaker position than `caching.md`'s, and the difference is worth keeping straight: caching at least has `spring-boot-starter-cache` on the classpath (`pom.xml:56`) waiting to be switched on, whereas resilience has no library at all, so adopting it starts with a dependency — and therefore an ADR, not just an annotation
