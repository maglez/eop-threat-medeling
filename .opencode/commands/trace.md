...
4. If anything failed, propose the concrete remedy — usually re-dispatching the missing stage-2 agents, or routing authoring work to the `MODEL_C` agents (`db-designer`, `devops-engineer`, `performance-engineer`, `tester-api`, `tester-unit-and-quality`) rather than writing it in the primary session.
